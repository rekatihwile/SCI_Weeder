...
class B1ProductionMission:
    ...
    def load_hardware_config(self):
        """Reads the JSON file generated by hardware_detect.py"""
        if not HARDWARE_CONFIG.exists():
            print(f"❌ ERROR: {HARDWARE_CONFIG} not found. Run hardware_detect.py first!")
            sys.exit()
        ...
        self.left_id = cfg['cameras']['left']
        self.right_id = cfg['cameras']['right']

        if not self.port or self.left_id == -1 or self.right_id == -1:
            print("❌ ERROR: Hardware config is incomplete. Check connections and re-run hardware_detect.py")
            sys.exit()

    def apply_nuclear_hardware_lock(self):
        """Applies manual V4L2 settings to the detected camera indices."""
        try:
            with open(CAMERA_SETTINGS, 'r') as f:
                s = json.load(f)
            
            # Use the IDs we detected dynamically
            ...
            print("✅ Nuclear Lock Applied to detected cameras.")
        except Exception as e:
    ...
    def match_and_optimize(self):
        ...
        while rem:
            node = min(rem, key=lambda i: np.linalg.norm(np.array(matched[node]['orig_l']) - curr_xy))
