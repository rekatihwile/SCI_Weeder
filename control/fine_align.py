...
def debug_reid_target(
    ...
    matched_targets, debug_data = _burst_match_ai_points(
        ...
        reid_settings=settings,
        return_debug=True,
    )

    if not matched_targets:
        print("[RE-ID DEBUG] No matched stereo pairs found.")
        if show_debug:
            _show_reid_debug_window(debug_data, [], target_idx=target_idx, total_targets=total_targets)
        return None

    candidates, rejects = _rank_reid_matches(
        gantry,
        coarse_mover,
        planned_target,
        actual_hits,
    ...
    if not candidates:
        print(
            "[RE-ID DEBUG] No candidate survived ranking filters: "
    ...
    _print_reid_ranked(candidates, header="[RE-ID DEBUG RANKED] no-motion candidate choices:")
    if show_debug:
        _show_reid_debug_window(debug_data, candidates, target_idx=target_idx, total_targets=total_targets)

    best_solved, best_pd_err, best_tri_dist, best_geo = candidates[0]
    src = best_solved["source_target"]
    print(
        f"[RE-ID DEBUG OK] would choose L={src['left_px']} R={src['right_px']} "
        f"tri_dist={best_tri_dist:.1f}mm pd_err={best_pd_err:.1f}px geo={best_geo:.2f}"
    )
    return best_solved
...
def _pick_best_ai_target(
    ...
    matched_targets = _burst_match_ai_points(cameras, detector, expected_cls=expected_cls, reid_settings=settings)
    if not matched_targets:
        print("[RE-ID FAIL] No matched stereo pairs found in burst.")
    ...
    if not candidates:
        print(f"[RE-ID FAIL] No candidates survived ranking filters ({settings['max_pd_error_px']}px PD limit).")
        _fine_debug(
            f"initial target pick rejected {len(matched_targets)} match(es): "
            f"crop={rejects['crop']}, duplicate={rejects['duplicate']}, "
    ...
    src = best_solved["source_target"]
    print(
        f"[RE-ID OK] {len(candidates)} candidate(s) — "
        f"picked tri_dist={best_tri_dist:.1f}mm, pd_err={best_pd_err:.1f}px, geo={best_geo:.2f}"
    )
