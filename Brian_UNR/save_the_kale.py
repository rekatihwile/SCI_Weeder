...
class B1ProductionMission:
    ...
    def execute_precision_strike(self, return_pos, lk_L, lk_R):
        ...
        # ---------------------------
        # ---------------------------
        KALE_SAFETY_ON = True  # Toggle to False for "Aggressive Mode"
        WEED_CONF = 0.10
        KALE_CONF = 0.80
        # ---------------------------
        # ---------------------------
        ...
        print(f"⚔️ ENTERING PRECISION STRIKE (Safety: {'ON' if KALE_SAFETY_ON else 'OFF'})")
        self.laser.stop()
        time.sleep(0.1) 
        ...
        for _ in range(SAMPLES):
            ...
            # Tuned precision inference: Weeds=0.20, Kale=0.70 (based on your test results)
            boxes_L, _ = self.cv_L._get_detections(roi_L, weed_classes=[0, 2], kale_class=1, 
                                                   kale_thresh=0.05, weed_conf=WEED_CONF, kale_conf=KALE_CONF)
            boxes_R, _ = self.cv_R._get_detections(roi_R, weed_classes=[0, 2], kale_class=1, 
                                                   kale_thresh=0.05, weed_conf=WEED_CONF, kale_conf=KALE_CONF)

            # Check for Kale overlap if safety is enabled
            if KALE_SAFETY_ON:
                kale_found = False
                ...
                if kale_found:
                    kale_veto_count += 1
                    continue # Vetoed: skip keypoint extraction for this frame

            # Extract keypoints only for weed boxes containing the target
            pts_L = self.cv_L.return_full(roi_L, precalc_boxes=boxes_L)
            pts_R = self.cv_R.return_full(roi_R, precalc_boxes=boxes_R)
        ...
        # --- 2. THE ULTIMATUM ---
        if KALE_SAFETY_ON and kale_veto_count >= 3:
            print(f"🛑 KALE VETO! Strike Aborted ({kale_veto_count}/10 warnings).")
            # Safe return travel
        ...
        self.laser.send_raw("M4") # Enable Dynamic Power
        self.laser.spiral_burn(curr_pos['x'], curr_pos['y'], radius=4.0, steps=20, speed=1000)
        self.laser.send_raw("M5") # Safe Mode
        
        # --- 5. RECOVERY ---
        self.laser.set_acceleration(NORMAL_ACCEL/2)
        self.laser.send_raw(f"G90\nG1 X{return_pos['x']} Y{return_pos['y']} F{1000}")
        self.wait_for_idle(timeout=2.0)
    ...
    def start(self):
        try:
            ...
            while True:
                # 1. We now store pre-processed coordinate histories instead of raw images
                ...
                for _ in range(10):
                    ...
                    # Keep the natural 0.1s delay so the plant has time to move between frames
                    time.sleep(0.1)                        
                
                if len(burst_history_L) < 3: continue
                
                # 2. Use the new pure-math function instead of the old all-in-one function
