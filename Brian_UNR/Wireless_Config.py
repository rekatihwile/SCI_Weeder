...
def main():
    ...
    while True:
        # 1. Dynamic Camera Hardware Update
        ...
        if ret_l and ret_r:
            if not STREAM_ONLY:
                # 2. Process AI for both cameras
                for ai, frame in [(ai_L, f_l), (ai_R, f_r)]:
                    # Run detection with your tuned "Survey" thresholds
                    boxes, masks = ai._get_detections(
                        frame, weed_classes=[0, 2], kale_class=1, 
                        weed_conf=0.40, kale_conf=0.70 # Survey thresholds
                    )
                    # Get the pinpoint target dots
                    targets = ai.return_full(frame, precalc_boxes=boxes, precalc_masks=masks)

                    # --- DRAW KALE (Green Boxes) ---
                    if hasattr(ai, 'kale_boxes'):
                        for k_box in ai.kale_boxes:
                            b = k_box.xyxy[0].cpu().numpy().astype(int)
                            cv2.rectangle(frame, (b[0], b[1]), (b[2], b[3]), (0, 255, 0), 2)
                            cv2.putText(frame, "KALE", (b[0], b[1]-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

                    # --- DRAW WEEDS (Blue Boxes + Targets) ---
                    if hasattr(ai, 'filtered_boxes'):
                        for w_box in ai.filtered_boxes:
                            b = w_box.xyxy[0].cpu().numpy().astype(int)
                            cv2.rectangle(frame, (b[0], b[1]), (b[2], b[3]), (255, 100, 0), 2)

                    # Draw the Red/White Crosshairs for the final targets
                    for (x, y) in targets:
                        cv2.circle(frame, (x, y), 8, (0, 0, 255), -1) # Red center
                        cv2.drawMarker(frame, (x, y), (255, 255, 255), cv2.MARKER_CROSS, 15, 2)

            with lock:
                # Stack images side-by-side for the dashboard
