...
def process_frame_visuals(frame, detections, side_config):
    """Draws bounding boxes and HUD directly onto the frame."""
    
    # --- Color Dictionary (B, G, R format) ---
    # Customize these colors for your specific IDs
    colors = {
        0: (0, 0, 255),    # Red (Weed 1)
        1: (0, 255, 0),    # Green (Kale)
        2: (0, 165, 255),  # Orange (Weed 2)
    }
    fallback_color = (255, 255, 255) # White for unknown IDs

    for det in detections:
        # Scenario A: WeedCV returns bounding boxes [x1, y1, x2, y2, cls_id]
        if len(det) >= 5:
            x1, y1, x2, y2, cls_id = [int(val) for val in det[:5]]
            
        # Scenario B: WeedCV only returns center points [x, y, cls_id]
        else:
            x, y = [int(val) for val in det[:2]]
            cls_id = int(det[2]) if len(det) >= 3 else 0
            
            # Create a fixed-size 40x40 box around the center point
            box_size = 20 
            x1, y1 = x - box_size, y - box_size
            x2, y2 = x + box_size, y + box_size
            
        # Get the color for this specific ID
        color = colors.get(cls_id, fallback_color)
        ...
        label = f"ID: {cls_id}"
        # Ensure the label doesn't get cut off at the top of the screen
        label_y = max(15, y1 - 8) 
    ...
    for i, text in enumerate(hud):
        cv2.putText(frame, text, (15, 30 + (i * 30)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
