...
def main():
    ...
    if args.scale != 1.0:
        x_center = (args.x_max + args.x_min) / 2.0
        y_center = (args.y_max + args.y_min) / 2.0

        new_width = (args.x_max - args.x_min) * args.scale
        new_height = (args.y_max - args.y_min) * args.scale

        args.x_min = x_center - (new_width / 2.0)
        args.x_max = x_center + (new_width / 2.0)
        args.y_min = y_center - (new_height / 2.0)
        args.y_max = y_center + (new_height / 2.0)

        print(f"\n[INFO] Scaled grid area to {args.scale * 100:.0f}% of original size.")
        print(f"[INFO] New scaled X bounds: {args.x_min:.1f} to {args.x_max:.1f}")
        print(f"[INFO] New scaled Y bounds: {args.y_min:.1f} to {args.y_max:.1f}")
    # --------------------------------
    ...
    manifest["session"] = {
        ...
        "scale_applied": args.scale,  # Log the scale used for record-keeping
        "home_point": {
