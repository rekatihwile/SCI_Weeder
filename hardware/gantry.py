...
# -----------------------------------------------------------------------------
# Standalone gantry utility
# -----------------------------------------------------------------------------
def _gantry_cli_main():
    """Simple terminal control for move-to-coordinate and raw G-code testing."""
    import argparse
    sys.path.append(str(Path(__file__).resolve().parent.parent))

    from config import GRBL_PORT

    parser = argparse.ArgumentParser(description="GRBL gantry utility")
    parser.add_argument("--port", default=GRBL_PORT)
    parser.add_argument("--home", action="store_true")
    args = parser.parse_args()

    g = Gantry(port=args.port)
    try:
        if args.home:
            g.home()
        print("Commands: move X Y [feed], raw GCODE, pos, home, quit")
        while True:
            s = input("gantry> ").strip()
            if not s:
                continue
            if s in {"q", "quit", "exit"}:
                break
            if s == "home":
                ...
                print(g.send_raw(s[4:]))
            elif s.startswith("move "):
                parts = s.split()
                x = float(parts[1]); y = float(parts[2])
                feed = float(parts[3]) if len(parts) > 3 else 12000
