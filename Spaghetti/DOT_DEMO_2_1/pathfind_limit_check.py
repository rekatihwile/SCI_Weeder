...
class B1ProductionMission:
    ...
    def is_at_limit(self, mpos):
        """Polls the gantry position to see if it has hit the software bounds."""
        if not mpos: return False
        x, y = mpos['x'], mpos['y']
        # Check if we are within 1mm of the defined limits
        if x <= X_LIMITS[0]+1 or x >= X_LIMITS[1]-1 or y <= Y_LIMITS[0]+1 or y >= Y_LIMITS[1]-1:
            return True
        return False
    ...
    def save_current_as_anchor(self, mpos):
        ...
        print(f"💾 [ANCHOR] ID:{anchor_id} saved.")

    def match_and_optimize(self):
        print("🔍 Overlap Search & Pathfinding...")
        ...
        for m_id, (l_idx, r_idx) in enumerate(best_matches.items()):
            ...
            initial_snapshot[m_id] = {
                'l': matched[m_id]['l'].copy(), 'r': matched[m_id]['r'].copy(), 
                'orig_l': p_l, 'mag': np.sqrt(ex**2+ey**2)
            }
    ...
    def start(self):
        ...
        while True:
            ...
            if self.mode == "SURVEY":
                cv2.putText(fL, "SURVEY MODE", (10, 30), 0, 0.7, (255, 0, 255), 2)
                ...
                if self.mode == "EXECUTE":
                    ...
                    if tid not in self.lk_targets:
                        ...
                        if best_k:
                            ...
                            print(f"⏪ [GOLD RECOVERY] ID:{best_k}"); self.laser.set_acceleration(RECOVERY_ACCEL)
                    ...
                    if mag > DEADZONE: 
                        # --- DYNAMIC LIMIT CHECK ---
                        mpos = self.laser.update_status()
                        if self.is_at_limit(mpos):
                            print(f"🛑 GANTRY LIMIT REACHED @ ID {tid}. Skipping."); 
                            self.laser.stop()
                    ...
                    if time.time() - self.last_status_check > 0.4:
                        self.last_status_check = time.time()
                        if "Idle" in (self.laser.send_raw("?") or ""):
                            if self.recovery_settle_start is None: self.recovery_settle_start = time.time()
                            if time.time() - self.recovery_settle_start > 0.5:
                                for _ in range(5): self.cap_L.grab(); self.cap_R.grab()
                                _, fL_n = self.cap_L.read(); _, fR_n = self.cap_R.read()
                                anc = self.spatial_anchors[self.current_anchor_id]
                                self.lk_targets = {}
